...
## Market basket analysis - Year gender

year_gender_classname_rules <- sapply(
  unique(df_loyalty_basket_classname$year[!is.na(df_loyalty_basket_classname$year)]), function(x){
    nn <- x
    gender <- unique(df_loyalty_basket_classname$gender[!is.na(df_loyalty_basket_classname$gender)])
    out_ <- sapply(gender, function(y){
      data <- df_loyalty_basket_classname %>%
        dplyr::select(-c(customer_type, age_group, county_name)) %>%
        tidyr::drop_na(year, gender) %>%
        dplyr::filter(year == nn, gender == y) %>%
        dplyr::select(-c(year, gender)) %>%
      ...
      rules <- arules::apriori(transactions,
                               parameter = list(support = support_classname_year_gender, 
                                                confidence = confidence_classname_year_gender,
                                                minlen = 2,
                                                target = "rules"
                                                ),
...
df_year_gender_classname_rules <- do.call("rbind", year_gender_classname_rules) %>%
