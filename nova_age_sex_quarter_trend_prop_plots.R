...
## Quarter Age-gender Time series item count proportion plots
### nova by Age-gender
nova_age_sex_quarter_count_prop_plot <- 
  ...
  df_max <- df_loyalty_analysis %>%
    ...
    mutate(total = n()) %>%
    ungroup() %>%
    group_by(across(c(quarter_date, gender, total, age_group, nova))) %>%
...
print(nova_age_sex_quarter_count_prop_plot)  


## Combined Quarter age-gender time series plots 
### nova by Age-gender item count
nova_age_sex_quarter_count_prop_plot_grid <- ggpubr::annotate_figure(
  ggpubr::ggarrange(plotlist = nova_age_sex_quarter_count_prop_plot,
                    ncol = NULL,
                    nrow = length(nova_age_sex_quarter_count_prop_plot),
                    labels = names(nova_age_sex_quarter_count_prop_plot),
...
nova_age_sex_quarter_count_prop_plot_grid


## Saving the Monthly age-sex grid plots
ggsave(plot=nova_age_sex_quarter_count_prop_plot_grid, height = 7, width = 12,
       filename = paste0("nova_age_sex_quarter_count_prop_plot",".png"),
