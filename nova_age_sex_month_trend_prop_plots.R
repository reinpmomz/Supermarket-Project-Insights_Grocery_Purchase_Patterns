...
## Monthly Age-gender Time series item count proportion plots
### nova by Age-gender
nova_age_sex_month_count_prop_plot <- 
  ...
  df_max <- df_loyalty_analysis %>%
    tidyr::drop_na(gender, nova, age_group) %>%
    group_by(across(c(age_group, month_date, gender))) %>%
    mutate(total = n()) %>%
    ungroup() %>%
    group_by(across(c(month_date, gender, total, age_group, nova))) %>%
    summarise(count = n(), .groups = "drop") %>%
    mutate(prop = round(count/total, 3))
  
  df_new <- df_loyalty_analysis %>%
    tidyr::drop_na(gender, nova) %>%
...
print(nova_age_sex_month_count_prop_plot)  


## Combined Monthly age-gender time series plots 
### nova by Age-gender item count
nova_age_sex_month_count_prop_plot_grid <- ggpubr::annotate_figure(
  ggpubr::ggarrange(plotlist = nova_age_sex_month_count_prop_plot,
                    ncol = NULL,
                    nrow = length(nova_age_sex_month_count_prop_plot),
                    labels = names(nova_age_sex_month_count_prop_plot),
...
nova_age_sex_month_count_prop_plot_grid


## Saving the Monthly age-sex grid plots
ggsave(plot=nova_age_sex_month_count_prop_plot_grid, height = 7, width = 12,
       filename = paste0("nova_age_sex_month_count_prop_plot",".png"),
