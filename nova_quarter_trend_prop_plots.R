...
### nova 
nova_quarter_count_prop_plot <- line_sum_plot(df=df_loyalty_analysis %>%
                                         tidyr::drop_na(nova) %>%
                                         group_by(quarter_date) %>%
                                         mutate(total = n()) %>%
                                         ungroup() %>%
                                         group_by(across(c(quarter_date, total, nova))) %>%
...
print(nova_quarter_count_prop_plot)

ggtheme_descriptive_plot(angletext_xaxis = 90, striptext_size_x = 9)
### nova by gender, age_group, county
nova_quarter_count_by_prop_plot <- sapply(c("gender", "age_group", "county_name"), function(x){ 
  
  nn <- x
  
  df_new <- df_loyalty_analysis %>%
    tidyr::drop_na(nova, any_of(nn)) %>%
    group_by(across(c(quarter_date, any_of(nn)))) %>%
    mutate(total = n()) %>%
    ungroup() %>%
    group_by(across(c(quarter_date, any_of(nn), total, nova))) %>%
    summarise(count = n(), .groups = "drop") %>%
    mutate(prop = round(count/total, 3))
...
print(nova_quarter_count_by_prop_plot)  


## Combined Quarter time series plots 
### nova proportion item counts by gender, age_group, county

nova_quarter_count_by_prop_plot_grid <- ggpubr::annotate_figure(
  ggpubr::ggarrange(plotlist = nova_quarter_count_by_prop_plot,
                    ncol = NULL,
                    nrow = length(nova_quarter_count_by_prop_plot),
                    labels = c("Gender", "Age group (years)", "Location"), #names(nova_quarter_count_by_prop_plot)
                    hjust = c(-0.5, -0.3, -0.5),
                    vjust = 2.5,
...
nova_quarter_count_by_prop_plot_grid


ggtheme_descriptive_plot()
## Saving the Quarter grid plots
### nova item counts
for (j in seq(length(nova_quarter_count_prop_plot))) {
  ggsave(plot=nova_quarter_count_prop_plot[[j]], height = 5, width = 9,
         filename = paste0(nova_quarter_count_prop_plot[[j]][["labels"]][["title"]],"_quarter_count_prop_plot",".png"),
...
ggsave(plot=nova_quarter_count_by_prop_plot_grid, height = 7, width = 12,
       filename = paste0("nova_quarter_count_by_prop_plot",".png"),
       path = output_prop_plots_Dir, bg='white')
