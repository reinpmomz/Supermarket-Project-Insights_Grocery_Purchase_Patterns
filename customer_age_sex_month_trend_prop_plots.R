...
## Monthly Age-gender Time series customer plots
### Proportion by Age-gender

customer_age_sex_month_prop_plot <- line_sum_plot(
  df= df_loyalty_analysis %>%
    tidyr::drop_na(age_group, gender) %>%
    distinct(customer_id, month_date, .keep_all = TRUE) %>%
    group_by(month_date, gender) %>%
    mutate(total = n()) %>%
    ungroup() %>%
    group_by(across(c(month_date, gender, total, age_group ))) %>%
...
print(customer_age_sex_month_prop_plot)  

## Saving the Monthly age-sex customer plots
for (j in seq(length(customer_age_sex_month_prop_plot))) {
  ggsave(plot=customer_age_sex_month_prop_plot[[j]], height = 5, width = 9,
         filename = paste0("customer_age_sex_month_prop_plot_",j,".png"),
