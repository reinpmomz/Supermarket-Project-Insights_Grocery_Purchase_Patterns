...
## Quarter Age-county Time series item count proportion plots
### nova by Age-county
nova_age_county_quarter_count_prop_plot <- 
  ...
  df_max <- df_loyalty_analysis %>%
    tidyr::drop_na(county_name, nova, age_group) %>%
    group_by(across(c(age_group, quarter_date, county_name))) %>%
    mutate(total = n()) %>%
    ungroup() %>%
    group_by(across(c(quarter_date, county_name, total, age_group, nova))) %>%
...
print(nova_age_county_quarter_count_prop_plot)  


## Combined Quarter age-county time series plots 
### nova by Age-county item count
nova_age_county_quarter_count_prop_plot_grid <- ggpubr::annotate_figure(
  ggpubr::ggarrange(plotlist = nova_age_county_quarter_count_prop_plot,
                    ncol = NULL,
                    nrow = length(nova_age_county_quarter_count_prop_plot),
                    labels = names(nova_age_county_quarter_count_prop_plot),
...
nova_age_county_quarter_count_prop_plot_grid


## Saving the Monthly age-county grid plots
ggsave(plot=nova_age_county_quarter_count_prop_plot_grid, height = 7, width = 12,
       filename = paste0("nova_age_county_quarter_count_prop_plot",".png"),
