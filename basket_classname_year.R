...
## Market basket analysis - Year

year_classname_rules <- sapply(
  unique(df_loyalty_basket_classname$year[!is.na(df_loyalty_basket_classname$year)]), function(x){
    ...
    data <- df_loyalty_basket_classname %>%
      dplyr::select(-c(customer_type, gender, county_name, age_group)) %>%
      tidyr::drop_na(year) %>%
      dplyr::filter(year == nn) %>%
      dplyr::select(-c(year)) %>%
    ...
    rules <- arules::apriori(transactions,
                             parameter = list(support = support_classname_year,
                                              confidence = confidence_classname_year,
                                              minlen = 2,
                                              target = "rules"
                                              ),
...
df_year_classname_rules <- do.call("rbind", year_classname_rules) %>%
